...
if (len(sys.argv) < 2):
  print "Usage: python calc_largest_coupling_timestep_X100_baroclinicinstability.py <base_output_directory>"
  exit()
outpath = sys.argv[1]
...
for method in methodList:
  ...
  while (not success and dt != 0.0):
    if (dt % 10 != 0):
      dtString = '%0.2f' % (dt/100.0)
    elif (dt % 100 != 0):
      dtString = '%0.1f' % (dt/100.0)
    else:
      dtString = '%d' % (dt/100.0)
    fileName = outpath + '/output_tsteptype%d_tstep%s_dcmip4_X100/dcmip2012_test41.nc' \
  ...
  largestDtDict[method] = dt/100.0

filename = './data/largest_coupling_timestep_X100_baroclinicinstability.txt'
print('Writing out ' + filename)
fileobj = open(filename, 'w')
for method in methodList:
  fileobj.write('%11s %0.2f\n' % (method, largestDtDict[method]))
fileobj.close()
