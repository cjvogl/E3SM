...
if [[ $? == 0 ]]; then
  ...
  rm -f $WORKDIR/run_convergence_test.sh $WORKDIR/plot_convergence_test.py $WORKDIR/plot_time_stats.py
  rm -f $WORKDIR/plot_energy_error.py $WORKDIR/plot_energy_efficiency.py $WORKDIR/dictionaries.py
  rm -f $WORKDIR/plot_energy_error_from_sources.py
  ...
  ln -s $PWD/plot_scripts/plot_time_stats.py $WORKDIR/plot_time_stats.py
  ln -s $PWD/plot_scripts/plot_energy_error.py $WORKDIR/plot_energy_error.py
  ...
  ln -s $PWD/plot_scripts/plot_energy_error_from_sources.py $WORKDIR/plot_energy_error_from_sources.py
fi
