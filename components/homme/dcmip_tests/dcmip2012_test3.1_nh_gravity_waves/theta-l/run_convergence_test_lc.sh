...
if [[ $# -lt 1 ]]; then
  echo "USAGE: ./run_convergence_test.sh <tstep_type value> [rtol value] [-noHV]"
  exit -1
fi
...
if [[ $1 == 5 ]]; then
  ...
  TSTEP=(1.0 0.5 0.375 0.25 0.1875 0.125 0.0625)
  NMAX=(300 600 800 1200 1600 2400 4800)
fi

for n in ${!TSTEP[*]}; do
  ARGS="tsteptype $1 tstep ${TSTEP[$n]} nmax ${NMAX[$n]} nu 0.0"
  if [[ $# -gt 1 ]]; then
    ARGS="$ARGS rtol $2"
  fi
  if [[ $3 == "-noHV" ]]; then
