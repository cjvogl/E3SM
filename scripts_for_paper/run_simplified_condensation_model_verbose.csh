...
#
#   -  for every $testconfig you need a file cnvg_template_${testconfig}_verbose.csh
...
#set groupList = ("RKZ_SGR_qv0_ql0_Al0_lmt4_adjIC" \
#                 "RKZ_SGR_qv1_ql1_Al1_lmt4_adjIC" \
#                 "RKZ_SGR_extrapf_qv0_ql0_Al0_lmt4_adjIC" \
#                 "RKZ_SGR_extrapf_qv1_al1_Al1_lmt4_adjIC")
...
if ($MACH == "titan") then
   ...
   setenv PTMP     $S/$CCSMTAG/$taskname
   setenv EXELOC   $PTMP/exe
   set initDir = $S/$CCSMTAG/ne30_FC5_init/
...
if ($run_model > 0) then
  ...
  set atmInitList = (\
   ...
   set template = './cnvg_template_'$testconfig'_verbose.csh'
...
#set mfilt = 1                # 1 time slice per file 

set nhtfrq = 1  # output every timestep
set mfilt = `echo 1800 / $dtime | bc`  # maintain same number of files as 30 minute output
